...
pub fn sys_get_time(ts: *mut TimeVal, _tz: usize) -> isize {
    trace!("kernel: sys_get_time pid:{}", current_task().unwrap().pid.0);
    let us = get_time_us();
    let timeval = TimeVal {
        sec: us / 1_000_000,
        usec: us % 1_000_000,
...
pub fn sys_mmap(start: usize, len: usize, prot: usize) -> isize {
    trace!("kernel: sys_mmap pid:{}", current_task().unwrap().pid.0);
...
pub fn sys_munmap(start: usize, len: usize) -> isize {
    trace!("kernel: sys_munmap pid:{}", current_task().unwrap().pid.0);
    let start_va = VirtAddr::from(start);
    if !start_va.aligned() {
        return -1;
    }
